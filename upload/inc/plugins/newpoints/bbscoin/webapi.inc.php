...
} elseif ($mybb->input['action'] == "bbscoin_to_points") 
{
    ...
    try {
        $rsp_data = BBSCoinApiWebWallet::getTransactionDetails($mybb->settings['newpoints_bbscoin_walletd'], $transaction_hash);
    } catch (Exception $e) {
        $db->delete_query('newpoints_bbscoin_locks', "uid = '" . $mybb->user['uid'] . "'");
    ...
    if ($rsp_data['data']['status'] != 'normal' || $rsp_data['data']['confirmations'] <= $mybb->settings['newpoints_bbscoin_confirmed_blocks']) {
        $db->delete_query('newpoints_bbscoin_locks', "uid = '" . $mybb->user['uid'] . "'");
...
} elseif ($mybb->input['action'] == "points_to_bbscoin") 
{
    ...
    try {
        $rsp_data = BBSCoinApiWebWallet::send($mybb->settings['newpoints_bbscoin_walletd'], $mybb->settings['newpoints_bbscoin_wallet_address'], $real_price, $walletaddress, $orderid, $mybb->user['uid'], $need_point, $mybb->settings['newpoints_bbscoin_withdraw_fee'], false);
    } catch (Exception $e) {
        //newpoints_addpoints($mybb->user['uid'], $need_point);
        $db->delete_query('newpoints_bbscoin_locks', "uid = '" . $mybb->user['uid'] . "'");
        error('Error '.$e->getCode().','.$e->getMessage());
    ...
    if ($rsp_data['data']['hash']) {
        ...
        bbscoin_send_pm($lang->newpoints_bbscoin_withdraw_succ_title, $lang->sprintf($lang->newpoints_bbscoin_withdraw_succ, $rsp_data['data']['hash'], $need_point), $mybb->user['uid'], 1);

        $plugins->run_hooks("newpoints_bbscoin_points_to_bbscoins_succ");

        $db->delete_query('newpoints_bbscoin_locks', "uid = '" . $mybb->user['uid'] . "'");
        redirect($mybb->settings['bburl'] . "/member.php?action=profile&uid=".$mybb->user['uid'], $lang->sprintf($lang->newpoints_bbscoin_withdraw_succ, $rsp_data['data']['hash'], $need_point));
    } else {
        newpoints_addpoints($mybb->user['uid'], $need_point);
